...
const STOCKS = [
  ...
  "ITC.NS", "WIPRO.NS", "SBIN.NS", "BAJAJ-AUTO.NS", "HINDUNILVR.NS",
  "BHARTIARTL.NS", "LT.NS", "MARUTI.NS", "SUNPHARMA.NS", "ADANIPORTS.NS",
  "ULTRACEMCO.NS", "ASIANPAINT.NS", "KOTAKBANK.NS", "ONGC.NS", "POWERGRID.NS",
];
...
const StockTracker = () => {
  ...
  return (
    ...
    <div className="relative">
      <div className="z-20 absolute"><Sidebar/></div>
      <div>
      <div className="z-5 pl-52 pt-10 m-5">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5">
          {stockData.map((stock, index) => (
            <div
              key={index}
              className={`relative p-4 border rounded shadow-md bg-white ${stock.change >= 0 ? "border-green-500" : "border-red-500"
                }`}
            >
              {/* Top-right arrow */}
              <div
                className={`absolute top-2 right-2 ${stock.change >= 0 ? "text-green-500" : "text-red-500"
                  }`}
              >
